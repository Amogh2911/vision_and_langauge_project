...
def train(args, train_dataloader, model, criterion, optimizer, epoch, tokenizer, device):
    ...
    for i, (_, visual_features, q, labels, bbox_label) in enumerate(train_dataloader,0):
        ...
        if i % 10 == 0:  # Log every 10 batches
            print(f"Processing batch {i}/{len(train_dataloader)}", flush=True)
...
def validate(args, val_loader, model, criterion, epoch, tokenizer, device, save_output = False):
    ...
    with torch.no_grad():
        for i, (file_name, visual_features, q, labels, bbox_label) in enumerate(val_loader,0):
            ...
            if i % 10 == 0:  # Log every 10 batches
                print(f"Validating batch {i}/{len(val_loader)}", flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_dir', default= '/content/drive/MyDrive/checkpoints/lvit/',                           help='/vb/vbrm/lvit/') 
    ...
    if args.dataset_type == 'endovis':
        ...
        folder_head = 'dataset/EndoVis-18-VQLA/seq_'
        folder_tail = '/vqla/*.txt'
        ...
        # external dataloader
        external_folder_head = 'dataset/EndoVis-17-VQLA/'
        external_folder_tail = '/vqla/*.txt'        
